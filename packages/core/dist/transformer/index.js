...
function buildColorMap(colors) {
    ...
    m.set('bqdark', '#0D1117');
    m.set('bqblue', '#58A6FF');
    m.set('bqgreen', '#3FB950');
    m.set('bqorange', '#F78166');
    m.set('bqpurple', '#BC8CFF');
    m.set('bqgold', '#E3B341');
    m.set('bqsilver', '#8B949E');
    // Preamble \definecolor entries override built-ins
    for (const c of colors)
        m.set(c.name, c.hex);
    return m;
}
function resolveColor(name, ctx) {
...
// ── Label/ref two-pass ─────────────────────────────────────────────────────
function collectLabels(nodes, labels, currentText) {
    for (const n of nodes) {
        ...
        if (n.type === 'environment')
            collectLabels(n.body, labels, currentText);
        if (n.type === 'group')
            collectLabels(n.children, labels, currentText);
        if (n.type === 'command') {
            for (const arg of n.reqArgs)
                collectLabels(arg, labels, currentText);
        }
    }
}
// ── Section numbering ──────────────────────────────────────────────────────
function incrementSection(ctx, level) {
    if (level === 1) {
...
export function transform(ast, options = {}) {
    ...
    const envStyles = new Map();
    for (const [k, v] of Object.entries(DEFAULT_ENV_STYLES))
        envStyles.set(k, v);
    ...
    for (const n of ast.preamble.rawPreambleNodes) {
        if (n.type === 'command' && n.name === 'title')
            docTitle = flatText(n.reqArgs[0] ?? []);
        if (n.type === 'command' && n.name === 'author')
            docAuthor = flatText(n.reqArgs[0] ?? []);
        if (n.type === 'command' && n.name === 'date')
            docDate = flatText(n.reqArgs[0] ?? []);
    }
    const ctx = {
        colors: colorMap,
    ...
    if (ctx.docTitle)
        docMeta.title = ctx.docTitle;
    if (ctx.docAuthor)
        docMeta.author = ctx.docAuthor;
    if (ctx.docDate)
        docMeta.date = ctx.docDate;
    const docNode = {
        type: 'document',
        metadata: docMeta,
        children,
...
function isInlineNode(n) {
    ...
    if (n.type === 'comment')
        return true;
    if (n.type === 'command') {
        if (BLOCK_COMMANDS.has(n.name))
            return false;
        return true; // all other commands treated as inline
    }
    if (n.type === 'environment')
        return false; // environments are always block
    if (n.type === 'verbatim')
        return false;
    return false;
}
function transformNodes(nodes, ctx) {
    const result = [];
    let i = 0;
 ...
 * with spaces intact.
 */
function transformInlineSequence(nodes, ctx) {
    const result = [];
    for (const n of nodes) {
        if (n.type === 'comment')
            continue;
    ...
    while (result.length > 0 && result[0]?.type === 'text' && !result[0].content.trim()) {
        result.shift();
    }
...
function transformText(node) {
    ...
    if (/^\\*\d+(\.\d+)?\s*(pt|cm|mm|in|em|ex|pc|bp|dd|cc|sp)$/.test(trimmed))
        return null;
    // Keep whitespace-only nodes — they matter for inter-word spacing.
    return { type: 'text', content: node.content };
}
function transformMath(node) {
    if (node.kind === 'inline')
        return { type: 'mathInline', latex: node.content };
...
function transformGroup(node, ctx) {
    ...
    for (let i = 0; i < node.children.length; i++) {
        ...
        else if (fontWeight !== undefined && child.reqArgs.length === 0) {
            style.fontWeight = fontWeight;
            contentStart = i + 1;
        }
        else if (fontStyle !== undefined && child.reqArgs.length === 0) {
        ...
        else if (fontFamily !== undefined && child.reqArgs.length === 0) {
        ...
        else if (name === 'color' && child.reqArgs.length >= 1) {
            // \color{colorname} — declarative form (affects rest of group)
            const colorName = flatText(child.reqArgs[0] ?? []);
    ...
    if (hasStyle) {
        ...
        const styled = { type: 'styledText', children };
        const mutable = styled;
        if (style.fontSize !== undefined)
            mutable.fontSize = style.fontSize;
        if (style.fontWeight !== undefined)
            mutable.fontWeight = style.fontWeight;
        if (style.fontStyle !== undefined)
            mutable.fontStyle = style.fontStyle;
        if (style.fontVariant !== undefined)
            mutable.fontVariant = style.fontVariant;
        if (style.fontFamily !== undefined)
            mutable.fontFamily = style.fontFamily;
        if (style.color !== undefined)
            mutable.color = style.color;
        if (style.textAlign !== undefined)
            mutable.textAlign = style.textAlign;
        return styled;
    }
...
function transformCommand(node, ctx) {
    ...
    switch (name) {
        ...
        case 'section*': {
            ...
            const title = arg0.map(n => transformNode(n, ctx)).filter((n) => n !== null);
            ...
            const sNode = { type: 'section', level: 1, title, number, id, children: [] };
            return sNode;
        }
        case 'subsection':
        case 'subsection*': {
            ...
            const id = `section-${number.replace(/\./g, '-') || titleText.toLowerCase().replace(/\s+/g, '-').slice(0, 40)}`;
            if (number)
                ctx.tocEntries.push({ level: 2, number, title: titleText, id });
        ...
        case 'textcolor': {
            ...
            const children = arg1.map(n => transformNode(n, ctx)).filter((n) => n !== null);
            const c = { type: 'color', color: hex, children, style: { color: hex } };
            return c;
        }
        case 'colorbox':
        case 'fcolorbox': {
        ...
        case 'href': {
            ...
            const children = arg1.map(n => transformNode(n, ctx)).filter((n) => n !== null);
            const link = { type: 'link', href: url, children };
            return link;
        }
        case 'url':
        case 'hyperref': {
        ...
        case 'ref': {
            ...
            const ref = { type: 'reference', targetId, resolvedText };
            return ref;
        }
        case 'label':
            return null;
        ...
        case 'item': {
            const children = arg0.map(n => transformNode(n, ctx)).filter((n) => n !== null);
            const li = { type: 'listItem', children };
            return li;
        }
        ...
        case 'maketitle': {
            const tpMeta = {};
            if (ctx.docTitle)
                tpMeta.title = ctx.docTitle;
            if (ctx.docAuthor)
                tpMeta.author = ctx.docAuthor;
            if (ctx.docDate)
                tpMeta.date = ctx.docDate;
            const tp = {
                type: 'titlePage',
                children: [],
                metadata: tpMeta,
        ...
        case 'multicolumn': {
            ...
            const align2 = alignRaw2.startsWith('c') ? 'center' : alignRaw2.startsWith('r') ? 'right' : 'left';
...
function transformEnvironment(node, ctx) {
    const name = node.name;
    ...
    switch (name) {
        ...
        case 'itemize': {
            ...
            const list = { type: 'list', ordered: false, children: items };
            return list;
        }
        case 'enumerate': {
            const items = transformListItems(node.body, ctx);
            const list = { type: 'list', ordered: true, children: items };
            return list;
        }
        case 'tabular':
        case 'longtable':
        ...
        case 'figure':
        case 'figure*': {
            const children = node.body.map(n => transformNode(n, ctx)).filter((n) => n !== null);
            const fig = { type: 'figure', children };
            return fig;
        }
        case 'tikzpicture': {
        ...
        case 'center': {
            const children = transformNodes(node.body, ctx);
            const c = { type: 'center', children };
            return c;
        }
        case 'abstract': {
            const children = transformNodes(node.body, ctx);
            const a = { type: 'abstract', children };
            return a;
        }
        case 'titlepage': {
            const children = transformNodes(node.body, ctx);
            const tp = { type: 'titlePage', children };
            return tp;
        }
        case 'multicols':
        case 'minipage': {
        ...
        case 'lstlisting': {
            ...
            const code = { type: 'codeBlock', code: raw, language: lang.toLowerCase() };
            return code;
        }
        case 'verbatim':
        case 'Verbatim': {
        ...
        case 'flushleft': {
            const children = transformNodes(node.body, ctx);
            const styled = { type: 'styledText', textAlign: 'left', children };
            return styled;
        }
        case 'flushright': {
            const children = transformNodes(node.body, ctx);
            const styled = { type: 'styledText', textAlign: 'right', children };
            return styled;
        }
        case 'quote':
        case 'quotation': {
...
function transformVerbatim(node, ctx) {
    const langOpt = node.options.find(o => o.key === 'language');
    const lang = langOpt?.value ?? ctx['options'] ?? '';
...
function transformTable(node, ctx) {
    const rows = [];
    let caption;
    ...
    for (const rowNodes of rowGroups) {
        ...
        if (rowNodes.length === 1 && rowNodes[0]?.type === 'command' && rowNodes[0].name === 'caption') {
            const capCmd = rowNodes[0];
    ...
    for (const n of nodes) {
        ...
        if (n.type === 'text' && n.content.includes('\\\\')) {
            ...
            for (let i = 0; i < parts.length; i++) {
                const part = parts[i];
                if (part.trim())
                    current.push({ type: 'text', content: part, pos: n.pos });
                if (i < parts.length - 1) {
                    // End of row
                    if (current.length > 0) {
                        rows.push(current);
...
function splitTableCells(nodes) {
    const cells = [];
    let current = [];
    for (const n of nodes) {
...
// ── List transformer ──────────────────────────────────────────────────────
function transformListItems(nodes, ctx) {
    const items = [];
...
function flattenBodyToString(nodes) {
    return nodes.map(n => {
        if (n.type === 'text')
            return n.content;
        if (n.type === 'verbatim')
            return n.content;
        if (n.type === 'math')
            return n.kind === 'inline' ? `$${n.content}$` : `$$${n.content}$$`;
        if (n.type === 'command') {
            let s = '\\' + n.name;
            for (const opt of n.optArgs)
                s += '[' + flatText(opt) + ']';
            for (const req of n.reqArgs)
                s += '{' + flatText(req) + '}';
            return s;
        }
        if (n.type === 'environment')
            return `\\begin{${n.name}}${flattenBodyToString(n.body)}\\end{${n.name}}`;
        if (n.type === 'group')
            return '{' + flattenBodyToString(n.children) + '}';
        return '';
    }).join('');
}

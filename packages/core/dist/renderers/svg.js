...
function mkCtx(width, theme, colors) {
...
function push(ctx, el) { ctx.elements.push(el); }
function addVSpace(ctx, px) { ctx.y += px; }
// ── Render dispatch ───────────────────────────────────────────────────────
function renderNode(node, ctx) {
    switch (node.type) {
        case 'document':
            renderDocument(node, ctx);
            break;
        case 'section':
            renderSection(node, ctx);
            break;
        case 'paragraph':
            renderParagraph(node, ctx);
            break;
        case 'text':
            renderParagraphSegments([{ text: node.content, fill: ctx.fg, bold: false, italic: false, isMath: false }], ctx);
            addVSpace(ctx, 8);
            break;
        case 'inlineCode':
            renderParagraphSegments([{ text: node.content, fill: ctx.fg, bold: false, italic: false, isMath: false, mono: true }], ctx);
            addVSpace(ctx, 8);
            break;
        case 'bold':
            renderParagraphSegments(extractSegments(node.children, ctx.fg, true, false), ctx);
            addVSpace(ctx, 8);
            break;
        case 'italic':
            renderParagraphSegments(extractSegments(node.children, ctx.fg, false, true), ctx);
            addVSpace(ctx, 8);
            break;
        case 'underline':
            renderParagraphSegments(extractSegments(node.children, ctx.fg, false, false), ctx);
            addVSpace(ctx, 8);
            break;
        case 'color':
            renderParagraphSegments(extractSegments(node.children, node.color, false, false), ctx);
            addVSpace(ctx, 8);
            break;
        case 'mathInline':
            renderParagraphSegments([{ text: node.latex, fill: ctx.fg, bold: false, italic: true, isMath: true }], ctx);
            addVSpace(ctx, 8);
            break;
        case 'mathDisplay':
            renderMathDisplay(node.latex, ctx);
            break;
        case 'codeBlock':
            renderCodeBlock(node.code, node.language, node.caption, ctx);
            break;
        case 'table':
        ...
        case 'figure':
            ...
            break;
        case 'image':
            renderImagePlaceholder(node.src, node.alt, node.width, ctx);
            break;
        case 'caption':
            renderCaption(node.children, ctx);
            break;
        case 'customBox':
            renderCustomBox(node.label, node.borderColor, node.bgColor, node.children, ctx);
            break;
        case 'link':
            renderLink(node.href, extractText(node.children), ctx);
            break;
        case 'reference':
            renderReference(node.resolvedText, node.targetId, ctx);
            break;
        case 'toc':
            renderToc(node.entries, ctx);
            break;
        case 'titlePage':
            renderTitlePageSvg(node, ctx);
            break;
        case 'center':
            renderCenteredChildren(node.children, ctx);
            break;
        case 'abstract':
            renderAbstract(node.children, ctx);
            break;
        case 'tikz':
            renderTikzPlaceholder(node.rawSource, ctx);
            break;
        case 'plot':
            renderTikzPlaceholder(node.rawSource, ctx);
            break;
...
function extractSegments(nodes, fill, bold = false, italic = false) {
    const segs = [];
    for (const n of nodes) {
        switch (n.type) {
            ...
            default: {
                ...
                if (t.trim())
                    segs.push({ text: t, fill, bold, italic, isMath: false });
            }
        }
    }
    return segs;
...
function renderParagraphSegments(segments, ctx) {
...
function renderDocument(node, ctx) {
...
function renderInlineText(text, ctx, fs, fill, bold, italic) {
    if (!text.trim())
        return;
...
function renderMathInline(latex, ctx) {
...
function renderMathDisplay(latex, ctx) {
    addVSpace(ctx, 8);
    const text = `\\[${latex}\\]`;
    const lines = wrapText(text, ctx.contentWidth, CHAR_W);
    for (const line of lines) {
...
function renderCodeBlock(code, language, caption, ctx) {
    addVSpace(ctx, 8);
...
function renderTable(node, ctx) {
    ...
    const dataRows = node.children.filter((r) => r.type === 'tableRow');
...
function renderTableRuleInSvg(rule, colCount, colW, ctx) {
...
function renderTableRowInSvg(row, _colCount, colW, rowH, ctx) {
    const rowTop = ctx.y;
...
function renderTableCellInSvg(cell, colIndex, colW, rowH, isHeader, ctx) {
...
function renderImagePlaceholder(src, alt, width, ctx) {
    addVSpace(ctx, 8);
...
function renderCaption(children, ctx) {
...
function renderCustomBox(label, borderColor, bgColor, children, ctx) {
    addVSpace(ctx, 8);
    const savedY = ctx.y;
    // Reserve space inside the box — we'll draw children first, then draw the rect
    const innerCtx = { ...ctx, elements: [], y: ctx.y + 12 };
...
function renderLink(href, text, ctx) {
    const lines = wrapText(text, ctx.contentWidth, CHAR_W);
    for (const line of lines) {
...
function renderReference(resolvedText, targetId, ctx) {
...
function renderToc(entries, ctx) {
    if (entries.length === 0)
        return;
    addVSpace(ctx, 8);
    const boxTop = ctx.y;
...
function renderCenteredChildren(children, ctx) {
    const text = extractText(children);
    if (!text.trim())
        return;
    const lines = wrapText(text, ctx.contentWidth, CHAR_W);
...
function renderAbstract(children, ctx) {
    addVSpace(ctx, 8);
    const boxTop = ctx.y;
    ...
    ctx.y += LINE_HEIGHT + 4;
    const innerCtx = { ...ctx, elements: [], y: ctx.y, contentWidth: ctx.contentWidth - 24 };
    for (const child of children) {
        renderNode(child, innerCtx);
    }
    ctx.elements.push(...innerCtx.elements);
...
function renderTikzPlaceholder(rawSource, ctx) {
    addVSpace(ctx, 8);
    const ph = 80;
...
export function renderToSvg(tree, options = {}) {

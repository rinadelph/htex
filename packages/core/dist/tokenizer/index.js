...
export function tokenize(source) {
    ...
    function push(kind, value, tl, tc) {
    ...
    while (i < source.length) {
        ...
        if (ch === '\\') {
            ...
            if (isAlpha(nx)) {
                let name = '';
                while (i < source.length && isAlpha(peek()))
                    name += advance();
                    ...
                    if (peek() === '{') {
                        ...
                        if (name === 'begin') {
                            if (VERBATIM_ENVS.has(envName)) {
                                ...
                                let body = '';
                                while (i < source.length && !source.startsWith(endMark, i))
                                    body += advance();
                                // Consume the \end{envName}
                                if (source.startsWith(endMark, i)) {
                                    for (let k = 0; k < endMark.length; k++)
                                        advance();
                                }
        ...
        if (ch === '&') {
            advance();
            push(TokenKind.AMPERSAND, '&', tl, tc);
            continue;
        }
        // ── Whitespace ───────────────────────────────────────────────────────
        if (ch === ' ' || ch === '\t' || ch === '\r') {
            let ws = '';
            while (i < source.length && ' \t\r'.includes(peek()))
                ws += advance();
        ...
        if (text)
            push(TokenKind.TEXT, text, tl, tc);
    }

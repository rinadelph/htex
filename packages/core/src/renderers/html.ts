...
import type {
  RenderNode, RenderTree, ColorDef, RenderOptions,
  TableRenderNode, SectionRenderNode, DocumentRenderNode,
  TitlePageRenderNode, InlineCodeRenderNode, UnderlineRenderNode,
} from '../types.js'
...
function escWithDashes(s: string): string {
  return esc(s)
    .replace(/---/g, '&mdash;')
    .replace(/--/g, '&ndash;')
...
function renderMathKatex(latex: string, display: boolean): string {
  try {
...
//
function buildCss(colors: readonly ColorDef[], theme: 'light' | 'dark'): string {
...
function renderFlatCanvas(
  ...
  for (const node of nodes) {
    if (node.type === 'document') {
      parts.push(renderFlatCanvas((node as DocumentRenderNode).children, theme, diagUrl))
      continue
    }
...
export function renderToHtml(
  ...
  for (const n of nodes) {
    if (n.type === 'document') {
      ...
      if (n.metadata.title) docTitle = n.metadata.title
    }
  }
...
function HTEX_PAGINATE_IMPL(root: Element | Document): void {
  ...
  const targets = Array.from(
    queryRoot.querySelectorAll<HTMLElement>('.htex-document[data-htex-unpaginated]'),
  )

  for (const docEl of targets) {
    const canvas  = docEl.querySelector<HTMLElement>('.htex-canvas')
    const pagesEl = docEl.querySelector<HTMLElement>('.htex-pages')
    if (!canvas || !pagesEl) continue
    ...
    type Bucket = HTMLElement[]
    const pages: Bucket[] = []
    let current: Bucket = []

    function pageBoxH(): number {
    ...
    function isWidow(el: HTMLElement): boolean {
    ...
    function wouldOverflow(el: HTMLElement): boolean {
      ...
      if (wouldOverflow(child)) {
        if (current.length === 0) {
          ...
          if (current.length > 1 && isWidow(last)) {
            ...
            if (kids.length > 0) pageBox.removeChild(kids[kids.length - 1]!)
            flushPage()
 ...
 * ```
 */
export function htexPaginate(root: Element | Document = document): void {
  HTEX_PAGINATE_IMPL(root)
}


// ── Node renderer dispatch ────────────────────────────────────────────────
function renderNode(node: RenderNode, theme: string, diagUrl = ''): string {
  switch (node.type) {
    ...
    case 'titlePage':   return renderTitlePage(node as TitlePageRenderNode, theme, diagUrl)
    ...
    case 'tikz':        return renderTikz(node as import('../types.js').TikzRenderNode, theme, diagUrl)
    case 'plot':        return renderTikz(node as import('../types.js').PlotRenderNode, theme, diagUrl)
    case 'hRule':       return `<hr>`
    case 'pageBreak':   return `<!-- page break -->`
    case 'styledText':  return renderStyledText(node as import('../types.js').StyledTextRenderNode, theme, diagUrl)
    default:            return ''
  }
}
...
function renderStyledText(
  node: import('../types.js').StyledTextRenderNode,
  theme: string,
  diagUrl = '',
): string {
  const styles: string[] = []
  ...
  if (node.fontStyle)    styles.push(`font-style:${node.fontStyle}`)
  if (node.fontVariant)  styles.push(`font-variant:${node.fontVariant}`)
  ...
  if (node.marginRight)  styles.push(`margin-right:${node.marginRight}`)
  if (node.visibility)   styles.push(`visibility:${node.visibility}`)
  if (node.whiteSpace)   styles.push(`white-space:${node.whiteSpace}`)
...
function renderTitlePage(node: TitlePageRenderNode, theme: string, diagUrl = ''): string {
  ...
  if (m.title)  html += `<div class="htex-title">${escWithDashes(m.title)}</div>\n`
  if (m.author) html += `<div class="htex-author">${escWithDashes(m.author)}</div>\n`
  if (m.date)   html += `<div class="htex-date">${escWithDashes(m.date)}</div>\n`
  html += `</div>\n`
  return html
}

// ── Document node ─────────────────────────────────────────────────────────
function renderDocument(node: DocumentRenderNode, theme: string, diagUrl = ''): string {
  return renderChildren(node.children, theme, diagUrl)
}

// ── Sections ─────────────────────────────────────────────────────────────
function renderSection(node: SectionRenderNode, theme: string, diagUrl = ''): string {
...
// ── Paragraphs ────────────────────────────────────────────────────────────
function renderParagraph(node: { children: readonly RenderNode[] }, theme: string, diagUrl = ''): string {
  const inner = renderChildren(node.children, theme, diagUrl)
...
// ── Code blocks ──────────────────────────────────────────────────────────
function renderCodeBlock(node: { code: string; language: string; caption?: string }): string {
...
// ── Tables ────────────────────────────────────────────────────────────────
function renderTable(node: TableRenderNode, theme: string, diagUrl = ''): string {
...
function renderTableCell(
  node: { colspan: number; rowspan: number; align: string; children: readonly RenderNode[] },
  theme: string, diagUrl = '',
): string {
  const attrs: string[] = []
  if (node.colspan > 1) attrs.push(`colspan="${node.colspan}"`)
  if (node.rowspan > 1) attrs.push(`rowspan="${node.rowspan}"`)
  if (node.align !== 'left') attrs.push(`style="text-align:${node.align}"`)
  const attrStr = attrs.length ? ' ' + attrs.join(' ') : ''
  return `<td${attrStr}>${renderChildren(node.children, theme, diagUrl)}</td>`
}

function renderTableRule(node: { ruleKind: string }): string {
  return `<tr class="htex-rule-${esc(node.ruleKind)}"><td colspan="100" style="padding:0;border:none"></td></tr>`
}

// ── Lists ─────────────────────────────────────────────────────────────────
function renderList(node: { ordered: boolean; children: readonly RenderNode[] }, theme: string, diagUrl = ''): string {
  const tag = node.ordered ? 'ol' : 'ul'
  return `<${tag}>${renderChildren(node.children, theme, diagUrl)}</${tag}>`
}

function renderListItemChildren(children: readonly RenderNode[], theme: string, diagUrl = ''): string {
  // Unwrap single paragraph inside list item to avoid double-indent
  if (children.length === 1 && children[0]?.type === 'paragraph') {
    return renderChildren((children[0] as { children: readonly RenderNode[] }).children, theme)
  }
...
function renderCustomBox(
  node: { label: string; borderColor: string; bgColor: string; children: readonly RenderNode[] },
  theme: string, diagUrl = '',
): string {
...
// ── Abstract ─────────────────────────────────────────────────────────────
function renderAbstract(node: { children: readonly RenderNode[] }, theme: string, diagUrl = ''): string {
  return `<div class="htex-abstract"><div class="htex-abstract-title">Abstract</div>${renderChildren(node.children, theme, diagUrl)}</div>`
}

// ── TOC ───────────────────────────────────────────────────────────────────
function renderToc(node: { entries: readonly { level: number; number: string; title: string; id: string }[] }, _theme: string): string {
...
function renderTikz(
  node: import('../types.js').TikzRenderNode | import('../types.js').PlotRenderNode,
  _theme: string,
  diagUrl = '',
): string {
  ...
  const fillMatch = src.match(
    /\\fill\[([^\]]+)\]\s*\(([^)]+)\)\s*rectangle\s*\(([^)]+)\)/
  )
  if (fillMatch) {
...
// ── Children ──────────────────────────────────────────────────────────────
function renderChildren(children: readonly RenderNode[], theme: string, diagUrl = ''): string {
